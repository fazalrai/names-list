    ...
    describe "create" do
        ...
        context "when authenticated" do
            ...
            it "returns a message if invalid" do
                ...
                expect { post :create, format: :json, params: { todo_item: invalid_new_todo } }.to_not change{ Name.count }
    ...
    describe "destroy" do
        ...
        context "when authenticated" do
            ...
            it "destroys a todo_item" do
                sign_in user_with_names
                destroyed_todo = user_with_names.names.first
                expect{ delete :destroy, format: :json, params: { id: destroyed_todo.id } }.to change{ Name.count }.by(-1) 
            end
            it "does not allow a user to destroy other's names" do
                sign_in user_with_names
                another_users_destroyed_todo = another_user_with_names.names.first
                expect{ delete :destroy, format: :json, params: { id: another_users_destroyed_todo.id } }.to_not change{ Name.count }
            end 
        end
        context "when not authenticated" do
            it "returns unauthorized" do
